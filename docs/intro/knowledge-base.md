...
知识库描述用于大模型逐库判断资料是否与当前问题相关，建议写清资料主题、适用问题和不包含的内容。未填写描述时暂按库名判断并提示补充，库名含糊时可能需要澄清。管理范围必须包含在读取范围内。
...
知识库选择只会缩小当前用户已有的可见范围，不能授予额外权限。新建对话后提出与检索测试相同的问题，并要求说明依据文件。聊天服务先根据问题与各库描述选择相关库并检索；没有相关库时不执行内容查询。智能体需要进一步查询或打开原文时，激活 `knowledge-base` Skill 使用对应工具。

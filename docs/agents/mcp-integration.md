...
SSE 和 Streamable HTTP 请求只访问配置 URL 所在的源站（协议、主机和端口相同）。同源重定向可正常工作；跨源重定向会在下一次请求发出前被拒绝，避免自定义认证头或请求内容泄露。遇到跨源跳转时，先核实最终服务的归属，再把配置 URL 改为该可信地址。此限制不代替内网访问控制或 DNS 安全策略。
...
每次工具执行前都会重新读取数据库中的服务器和工具启用状态。禁用或删除提交后，尚未通过执行检查的调用会返回工具错误，运行中的智能体持有的旧工具也受此限制；数据库读取失败时停止调用。已经通过检查或已在远端执行的调用不会被撤回。此规则同时适用于远程 MCP 和内置 stdio。

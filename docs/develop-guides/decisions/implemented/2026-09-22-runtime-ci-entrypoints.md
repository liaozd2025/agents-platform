...
Runtime 在 Docker 启动后安装宿主 CLI 工具时，uv 缓存仅扫描 `packages/yuxi-cli/pyproject.toml` 与 `packages/yuxi-cli/uv.lock`，避免默认全仓扫描进入容器创建的受限数据目录。远端失败样本为 run 35687023070 的 setup-uv EACCES；后端链路已通过，浏览器段仍须在修正后的完整 run 中验证。

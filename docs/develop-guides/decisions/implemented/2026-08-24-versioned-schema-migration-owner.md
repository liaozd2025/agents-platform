...
现有 `storage-migrator` 是 shipping 拓扑唯一的 Yuxi Schema 修改者，不新增迁移服务或框架。迁移器持有 PostgreSQL session advisory lock，并分别记录 `business` 与 `knowledge` 域。升级基线覆盖 fork business=1/2/3/4/5 以及上游 business=7，目标为 business=8、knowledge=3；迁移器一次执行完整业务收敛 SQL，再记录当前 schema 版本，知识域补齐当前结构。未版本化安装保留新库初始化路径；LangGraph checkpoint setup 完成后才记录 business 版本。当前版本重复运行跳过对应域 DDL，其他未知版本在领域 DDL 前明确失败。

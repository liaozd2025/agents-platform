...
- Run 终态或显式状态刷新后，当前 artifact 预览先丢弃按 `thread_id + path` 保存的旧缓存，再从实时 Workdir 重新读取；同名文件重生成不沿用上一次内容。

其中状态面板展示由 `web/src/components/AgentChatComponent.vue` 拥有，预览缓存与重新加载由 `web/src/components/AgentPanel.vue` 拥有，交付物预览授权与响应由 `backend/package/yuxi/services/artifact_service.py` 拥有。Artifact 只把已授权字节交给预览适配器，runtime Office 缓存仍由 [`yuxi.workspace.preview`](./2026-08-21-preview-owner-separation.md) 拥有。
...
继续同一子线程时，状态面板只显示最新一项，旧运行不会单独占行；最新调用没有任务描述时会保留同线程已有描述，全部缺失时显示 `child_thread_id`。悬浮面板自然贴合内容，可用最大高度随输入区变化，极低可用高度下优先保证不覆盖输入区。刷新操作在桌面端成为渐进显现的次要操作，在触屏设备上仍保持可见。同一路径的产物在新 Run 完成后会重新读取实时内容，不要求用户关闭旧预览标签。
...
| 同路径 artifact 重生成后不复用旧预览 | `agentPanelSections.test.js` 约束 Run 刷新先失效 artifact cache 再重新读取 | Passed |

...
- 临时 PostgreSQL Schema 中运行 `test_schema_migration_version.py` 与 `test_durable_task_repository.py`；覆盖 legacy baseline 与 handler version 0、旧版 business 与 knowledge 完整收敛、并发 claim/dedupe、数据库时钟、领域 callback 与文件行锁等待、迟到 owner、Durable Task 容量上限、所有 Task 终态 hook、评估 checkpoint fencing，以及超过 200 条任务的摘要。

...
- `build_prompt_with_context` 优先读取该运行时标志；标志缺失（如单测直接构造 Context）时回退到按内容判定，两条路径行为一致。
- 工作区 `AGENTS.md`/`USER.md` 仍作为补充设定拼在末尾，但不再影响身份归属。
...
- `docker compose exec -T api python -m pytest test/unit/agents -q`：146 passed。
- `docker compose exec -T api python -m pytest test/unit/agents/test_chatbot_prompt.py -q`：7 passed（两次负向实验均为 1 failed / 6 passed）。
- `git diff --check`：通过。
- 未执行项：页面端新建智能体后真实对话回归（需 UI 操作），以及子智能体的独立交互验证。

...
- 新增“角色与权限”管理：内置角色受保护，自定义角色支持创建、复制、修改、停用和审计，角色编辑器由路由设置页直接打开；用户可分配多角色并继承或收窄单次范围，`superadmin` 独占且变更需原因，系统保留至少一名有效超管。持有 `user:role_assign` 的非超管仅可在单条授权同时覆盖目标、角色功能权限和范围时转授；越域、拼接授权及权限或范围扩张均拒绝。新用户默认绑定内置 `user`，升级时会回填并删除 `users.role`，登录、OIDC 与用户接口只返回角色数组和有效权限。旧管理员的全局旁路语义已取消，统一由功能权限、组织管理域和资源共享范围判定。

...
- 部门升级为集团组织机构树：管理页支持树形浏览、父节点选择与子树移动，拒绝环、根节点变更及非叶删除；删除叶子时用户回落集团根。知识库、智能体、Skill 的组织授权按子树继承，选择器同步树形化并校验冗余及读管范围。名称改为同级唯一，管理员可选；OIDC 仅精确匹配唯一已有节点；列表以单次聚合统计有效用户数。

...
- 部门升级为集团组织机构树：`departments` 新增父节点、仅供展示的节点类型和物化路径，`id=1` 原地改造为集团根，存量节点自动挂到其下，可访问人群不变；名称唯一性由全局收敛为同级唯一，不同分子公司可各有一个「人力资源部」；创建组织节点可指定父节点，同时创建管理员改为可选（该账号仍是全局管理员）；删除仍有子节点的节点会被拒绝，删除叶子节点时其用户回落集团根。

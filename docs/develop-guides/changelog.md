...
- 新增“角色与权限”管理：服务端维护稳定的功能权限与数据范围目录，保护内置超级管理员、管理员和普通用户角色；超级管理员可创建、复制、修改和停用自定义角色，配置功能权限与默认组织子树，查看成员及结构化安全审计；仍有成员的角色必须先迁移成员。新增角色、权限、默认范围、用户分配及审计表，启动时幂等回填存量 `users.role`，新建用户同步写入角色分配并保留旧鉴权行为。

...
- 系统设置由居中弹窗升级为无过渡动画的全屏设置中心：桌面端采用分组侧栏、设置搜索、返回应用和用户身份摘要，内容区居中独立滚动；移动端使用顶部返回栏与横向导航，并保留原有权限和各设置模块行为。
- 新增“角色与权限”管理：内置角色受保护，自定义角色支持创建、复制、修改、停用和审计；用户可分配多角色并继承或收窄单次范围，`superadmin` 独占且变更需原因，系统保留至少一名有效超管。持有 `user:role_assign` 的非超管仅可在单条授权同时覆盖目标、角色功能权限和范围时转授；越域、拼接授权及权限或范围扩张均拒绝。新用户默认绑定内置 `user`，旧字段仅迁移期保留。
...
- 智能体查看与运行改用 `agent:use`，创建、编辑、删除及配置改用 `agent:manage`；资源可见性与管理权继续由智能体共享范围约束，缺少功能权限返回 `403`、越出资源范围返回 `404`，前端入口、页签和操作同步按有效权限显隐。
...
- Dashboard 改由 `dashboard:view` 独立授权，当前人员与组织统计按查看者管理域实时计算；历史对话、调用、反馈和审计保存事件组织快照并按快照过滤，旧数据按当前归属回填且明确标记为推算。组织筛选包含所选节点及全部后代，只展示授权子树及必要祖先，越域筛选按不可见处理。

...
- 新增“角色与权限”管理：内置角色受保护，自定义角色支持创建、复制、修改、停用及结构化审计；用户管理支持多个普通角色及继承或收窄单次范围，`superadmin` 独占，授予撤销需填写原因并保护最后一个有效超级管理员；请求级授权按同一角色分配原子匹配功能权限与数据范围，角色入口和操作由服务端有效权限驱动，变更从下一请求生效；本地与 OIDC 新用户默认绑定内置 `user`，迁移期保留旧字段。
- 部门升级为集团组织机构树：管理页默认展开，节点图标与工具栏均可展开/收起；用户筛选和归属选择使用组织树，并在重新进入用户管理时刷新。组织授权按子树继承并校验读管范围；拒绝环、根节点变更，以及删除含子节点或直属用户的节点；名称同级唯一，OIDC 仅精确匹配唯一已有节点。

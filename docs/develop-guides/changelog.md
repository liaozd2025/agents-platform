...
- 部门升级为集团组织机构树：管理页支持树形展开、父节点与展示类型选择；节点可连同子树移动，并拒绝环、集团根移动/删除及非叶删除；删除叶子时用户回落集团根。知识库、智能体与 Skill 的组织共享按子树向下继承，保持兄弟隔离与全局共享语义。名称同级唯一，创建管理员改为可选且信息不完整时明确拒绝；存量表缺集团根会中止迁移。OIDC 仅精确匹配唯一已有节点，不再创建节点。

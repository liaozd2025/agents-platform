#!/usr/bin/env bash
# 使用一次性环境运行持久重试预算 E2E，成功或失败都删除本次容器、网络和数据。
set -euo pipefail
export KB_E2E_API_IMAGE="${1:?Usage: bash run_attempt_budget.sh API_IMAGE PROVISIONER_IMAGE}"
...
export COMPOSE_PROJECT_NAME="attempt-budget-$(date +%s)-$$"
export KB_E2E_STATE_DIR
KB_E2E_STATE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/attempt-budget.XXXXXX")"
mkdir -p "$KB_E2E_STATE_DIR/threads" "$KB_E2E_STATE_DIR/projections"
compose=(docker compose --env-file /dev/null -p "$COMPOSE_PROJECT_NAME" -f "$KB_E2E_REPO_ROOT/backend/test/e2e/knowledge/compose.yaml" -f "$KB_E2E_REPO_ROOT/backend/test/e2e/runs/compose.attempt-budget.yaml")
cleanup() {
...
"${compose[@]}" exec -T api uv run --no-sync pytest --confcutdir=test/e2e/runs test/e2e/runs/test_attempt_budget.py -q

...
from yuxi.storage.postgres.models_business import Base, Department, OperationLog, Role, User
...
async def test_required_audit_failure_rolls_back_department_and_admin() -> None:
    ...
    try:
        async with factory() as db:
            with pytest.raises(ValueError, match="组织快照对应的用户不存在"):
                await create_department_with_admin(
                    db,
...
async def test_concurrent_initialization_has_exactly_one_atomic_winner() -> None:
    ...
    try:
        ...
        role_definition = next(role for role in BUILTIN_ROLES if role.code == "superadmin")
        async with factory.begin() as db:
            db.add(
                Role(
                    ...
                    is_builtin=True,
                    is_active=True,
                    default_scope_type=role_definition.default_scope_type,
                )
            )

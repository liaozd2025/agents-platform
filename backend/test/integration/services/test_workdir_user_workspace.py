...
from yuxi.storage.postgres.models_business import Base, Conversation, Project, User
...
async def test_conversations_share_workdir_only_through_project(monkeypatch, tmp_path: Path):
    ...
    try:
        async with factory() as db:
            ...
            db.add_all([User(username="user-1", uid="user-1", password_hash="test"), project])
            await db.flush()
    ...
    try:
        ...
        async with engine.begin() as connection:
            ...
            await connection.execute(
                text("ALTER TABLE conversations DROP CONSTRAINT uq_conversations_uid_creation_request_id")
            )
            await connection.execute(text("ALTER TABLE conversations DROP COLUMN creation_request_id"))
            await connection.execute(
                text(
                    "INSERT INTO users (username, uid, password_hash, login_failed_count, is_deleted) "
                    "VALUES ('user-early', 'user-early', 'unused', 0, 0)"
                )
            )
            await connection.execute(
                text(
                    ...
                    "organization_snapshot_inferred) "
                    "VALUES ('thread-early', 'user-early', 'main', 'active', false, "
                    "CAST(:metadata AS JSONB), false), "
                    "('thread-empty', 'user-early', 'main', 'active', false, '{}', false), "
                    "('thread.v0:legacy', 'user-early', 'main', 'active', false, '{}', false)"
                ),
                {
                    "metadata": json.dumps(
                        {

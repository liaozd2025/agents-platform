...
@pytest.mark.parametrize("business_version,knowledge_version", [(5, 1), (7, 2)])
async def test_merge_migration_retries_knowledge_after_business_commit(
    tmp_path, monkeypatch, business_version, knowledge_version
) -> None:
    ...
    monkeypatch.setattr(storage_migration, "migrate_runtime_storage_identity", lambda: None)
    monkeypatch.setattr(storage_migration, "runtime_storage_requires_quiescence", lambda: False)
    legacy = {"access_level": "user", "department_ids": [], "user_uids": ["owner"]}
    current = {"version": 2, "read_scope": legacy, "manage_scope": None}
    try:
        await manager.create_business_tables()
        ...
        monkeypatch.setattr(manager, "ensure_knowledge_schema", fail_knowledge)
        monkeypatch.setattr(storage_migration, "pg_manager", manager)
...
@pytest.mark.parametrize("starting_version", [1, 2, 5, 7])

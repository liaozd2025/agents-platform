...
@pytest.mark.parametrize(
    ("business_version", "upgrades_business"),
    [
        (storage_migration.BUSINESS_SCHEMA_VERSION, False),
        (storage_migration.BUSINESS_SCHEMA_VERSION - 1, True),
    ],
)
async def test_schema_ddl_only_runs_for_supported_business_upgrade(
    monkeypatch,
    business_version,
    upgrades_business,
):
    ...
    assert {"create_business", "create_knowledge", "knowledge_schema"}.isdisjoint(calls)
    business_upgrade_calls = {
        "business_schema",
        f"version:business:{storage_migration.BUSINESS_SCHEMA_VERSION}",
    }
    if upgrades_business:
        assert business_upgrade_calls <= set(calls)
    else:
        assert business_upgrade_calls.isdisjoint(calls)
    assert "checkpoint" not in calls
    assert f"version:knowledge:{storage_migration.KNOWLEDGE_SCHEMA_VERSION}" not in calls
    assert "converge:False" in calls


@pytest.mark.asyncio
@pytest.mark.parametrize("unsupported_version", [0, 6, storage_migration.BUSINESS_SCHEMA_VERSION + 1])

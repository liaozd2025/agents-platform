...
async def test_runtime_access_still_excludes_disabled_shared_skill(monkeypatch: pytest.MonkeyPatch):
    ...
    async def empty_personal_skills(_uid):
        return []
...
@pytest.mark.asyncio
async def test_read_personal_skill_file_rejects_symlink_swap_after_validation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    """校验后被替换的文件仍不得跟随链接读取。"""
    ...
    outside = tmp_path / "outside.txt"
    outside.write_text("secret", encoding="utf-8")
    original_resolve = svc._resolve_relative_path

    def swap_after_resolve(actual_skill_dir, relative_path, *, allow_root=False):
        resolved = original_resolve(actual_skill_dir, relative_path, allow_root=allow_root)
        target.unlink()
        target.symlink_to(outside)
        return resolved

    monkeypatch.setattr(svc, "_resolve_relative_path", swap_after_resolve)

    with pytest.raises(ValueError, match="越界访问"):
        await svc.read_personal_skill_file("user-1", "demo", "notes.txt")


@pytest.mark.asyncio
async def test_read_shared_skill_file_rejects_parent_swap_after_validation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    """共享 Skill 读取必须从持久源根逐层 no-follow。"""
    data_root = tmp_path / "skill-sources"
    shared_root = data_root / "shared"
    skill_dir = shared_root / "demo"
    skill_dir.mkdir(parents=True)
    (skill_dir / "notes.txt").write_text("safe", encoding="utf-8")
    outside_root = tmp_path / "outside"
    (outside_root / "demo").mkdir(parents=True)
    (outside_root / "demo/notes.txt").write_text("secret", encoding="utf-8")
    item = Skill(slug="demo", name="demo", description="", dir_path="shared/demo")

    async def fake_get_skill(_db, _operator, _slug: str):
        return item

    original_resolve = svc._resolve_relative_path

    def swap_parent_after_resolve(actual_skill_dir, relative_path, *, allow_root=False):
        resolved = original_resolve(actual_skill_dir, relative_path, allow_root=allow_root)
        shared_root.rename(data_root / "shared-original")
        shared_root.symlink_to(outside_root, target_is_directory=True)
        return resolved

    monkeypatch.setattr(svc, "get_management_readable_skill_or_raise", fake_get_skill)
    monkeypatch.setattr(svc, "_resolve_relative_path", swap_parent_after_resolve)

    with pytest.raises(ValueError, match="越界访问"):
        await svc.read_skill_file(None, slug="demo", relative_path="notes.txt", operator=_user())
...
async def test_skill_cards_keep_shadowed_shared_item_for_management(
    ...
    cards = await svc.list_skill_cards_for_user(None, _user("user-1"))

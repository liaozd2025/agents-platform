...
def test_create_sandbox_reports_address_pool_exhaustion_as_unavailable(monkeypatch):
    ...
    def create(*_args, **_kwargs):
        raise module.SandboxCapacityError("network", 256)
    ...
    assert response.status_code == 503
    assert response.json()["detail"] == {"code": "sandbox_capacity_exhausted", "scope": "network", "limit": 256}
...
def test_docker_backend_reports_dedicated_address_pool_exhaustion():
    ...
    with pytest.raises(module.SandboxCapacityError, match="sandbox network capacity exhausted"):
...
def test_docker_backend_serializes_create_and_delete_for_same_sandbox(monkeypatch):
    ...
    backend._lock = threading.RLock()
    backend._sandbox_locks = {}
    backend._require_resource_policy = lambda _container: None

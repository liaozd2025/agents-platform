...
from yuxi.storage.postgres.models_business import Base, Department, Project, User
...
async def session():
    ...
    async with factory() as db:
        department = Department(name="测试组织", node_type="group")
        db.add(department)
        await db.flush()
        department.path = f"/{department.id}/"
        db.add_all(
            [
                User(username="oidc-user", uid="oidc:user@example.com", password_hash="x", department=department),
                User(username="user-1", uid="user-1", password_hash="x", department=department),
            ]
        )
        await db.flush()
        project_owners = (
            ("11111111-1111-4111-8111-111111111111", "oidc:user@example.com"),
            ("33333333-3333-4333-8333-333333333333", "user-1"),
            ("44444444-4444-4444-8444-444444444444", "user-1"),
            ("55555555-5555-4555-8555-555555555555", "user-1"),
        )
        db.add_all(
            [
                Project(
                ...
                for project_id, uid in project_owners
            ]
        )
        await db.flush()
        yield db

...
from yuxi.config import get_runtime_dir, get_skill_data_dir, get_skill_projection_dir
...
async def read_personal_skill_file(uid: str, slug: str, relative_path: str) -> dict[str, Any]:
    ...
    content = _read_utf8_regular_file(
        workspace_paths.get_user_data_dir(),
        (
            "shared",
            workspace_paths.workspace_uid_dirname(uid),
            workspace_paths.WORKSPACE_DIR_NAME,
            "agents",
            "skills",
            slug,
...
def _read_utf8_regular_file(root: Path, parts: tuple[str, ...]) -> str:
    """从可信根 no-follow 读取 UTF-8 普通文件。"""

    try:
        with open_regular_file_fd(root, parts) as (file_fd, _file_stat):
...
async def read_skill_file(
    ...
    *,
    ...
    if not _is_text_path(target):
        raise ValueError("仅支持读取文本文件")
    read_root = skill_dir
    read_parts = PurePosixPath(rel).parts
    persisted_dir = Path(item.dir_path)
    if not persisted_dir.is_absolute():
        persisted_parts = PurePosixPath(str(item.dir_path).replace("\\", "/")).parts
        if not persisted_parts or ".." in persisted_parts:
            raise ValueError("非法路径：越界访问被拒绝")
        read_root = get_skill_data_dir()
        read_parts = (*persisted_parts, *read_parts)
    content = _read_utf8_regular_file(read_root, read_parts)

...
class Workspace:
    ...
    def cleanup_authorized_directory(
        self,
        path: str,
        *,
        root: str,
        ...
        """清理受限目录；保留指定真实子目录，并在空时删除目标目录。"""
        if any(not name or name in {".", ".."} or "/" in name or "\\" in name for name in preserve_directories):
            raise ValueError("preserved directory name must be one path component")
        self._require_within(path, root)
        base, parts = self._resolve_path(path)
        if not parts:
            raise ValueError("operation cannot target the Workspace root")

        parent_fd = self._open_directory(base, parts[:-1])
        directory_fd = None
        try:
            directory_fd = self._open_directory(parent_fd, (parts[-1],))
            directory_stat = os.fstat(directory_fd)
            for name in os.listdir(directory_fd):
                item_stat = os.stat(name, dir_fd=directory_fd, follow_symlinks=False)
                if name in preserve_directories and stat.S_ISDIR(item_stat.st_mode):
                    continue
                self._remove_entry(directory_fd, name, unlink_symlinks=True)
            if os.listdir(directory_fd):
                return False

            current_stat = os.stat(parts[-1], dir_fd=parent_fd, follow_symlinks=False)
            if (current_stat.st_dev, current_stat.st_ino) != (directory_stat.st_dev, directory_stat.st_ino):
                raise RuntimeError("authorized directory changed during cleanup")
            os.close(directory_fd)
            directory_fd = None
    ...
    @staticmethod
    def _open_directory(base: Path | int, parts: tuple[str, ...], *, create: bool = False) -> int:
        try:
    ...
    @classmethod
    def _remove_entry(cls, parent_fd: int, name: str, *, unlink_symlinks: bool = False) -> None:
        item_stat = os.stat(name, dir_fd=parent_fd, follow_symlinks=False)
        if stat.S_ISLNK(item_stat.st_mode):
            if unlink_symlinks:

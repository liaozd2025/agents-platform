...
- `router` 定义公开首页、登录、智能体、工作区、智能体管理、扩展和仪表盘路由，并负责认证、功能权限、资源范围和超级管理员守卫。
...
`/` 是公开首页；登录后的核心工作区是 `/agent`。`/extensions` 对所有登录用户开放，其中知识库、工具和 MCP 等入口按有效功能权限展示；Dashboard 入口同样由功能权限控制。后端权限检查始终是最终边界，前端守卫只负责页面体验。

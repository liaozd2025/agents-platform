...
export const useTaskerStore = defineStore('tasker', () => {
  ...
  function createTaskRegistration() {
    const generation = sessionGeneration
    return (task) => {
      if (generation !== sessionGeneration || !userStore.hasPermission('system_task:manage')) return
  ...
  watch([() => userStore.token, () => userStore.effectivePermissions], reset, { flush: 'sync' })

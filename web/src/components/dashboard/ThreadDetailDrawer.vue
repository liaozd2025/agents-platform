<template>
  ...
  >
    <template #extra>
      <a-tag :color="getAgentRunStatusView(detail?.run_status).color">
        {{ getAgentRunStatusView(detail?.run_status).label }}
      </a-tag>
    </template>
...
const open = async (threadId, departmentId = null) => {
  if (!threadId) return
  visible.value = true
  loading.value = true
  detail.value = null
  ...
  try {
    const data = await dashboardApi.getConversationDetail(threadId, departmentId)
    detail.value = data
  } catch (err) {

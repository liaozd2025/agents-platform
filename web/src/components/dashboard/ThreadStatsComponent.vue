      ...
      <div class="explorer-header">
        ...
        <div class="explorer-actions">
          ...
          >
            ...
            <a-select-option value="active">正常会话</a-select-option>
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'status'">
              <a-tag :color="getAgentRunStatusView(record.run_status).color">
                {{ getAgentRunStatusView(record.run_status).label }}
              </a-tag>
            </template>
...
import { getAgentRunStatusView } from '@/utils/agentRun'
...
const conversationColumns = [
  ...
  { title: '任务状态', key: 'status', width: '90px', align: 'center' },
...
const loadFilterOptions = async () => {
  try {
    filterOptions.value = await dashboardApi.getConversationFilterOptions(props.departmentId)
  } catch (err) {
...
const handleOpenDetail = (threadId) => {
  detailDrawerRef.value?.open(threadId, props.departmentId)
}

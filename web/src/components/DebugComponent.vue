    ...
    <div :class="['debug-panel-wrapper', { fullscreen: state.isFullscreen }]" ref="debugWrapper">
      ...
      <div class="panel-header">
        <div class="header-title-group">
          ...
          <span class="env-badge">{{ assignedRolesText }}</span>
        </div>

        <div class="header-quick-actions">
          <a-tooltip v-if="userStore.hasPermission('knowledge_base:read')" title="输出知识库状态">
            <a-button size="small" class="header-action-btn" @click="printDatabaseInfo">
              <template #icon><Database :size="13" /></template>
            </a-button>
          </a-tooltip>
          <a-tooltip v-if="userStore.hasPermission('agent:manage')" title="输出智能体状态">
            <a-button size="small" class="header-action-btn" @click="printAgentConfig">
              <template #icon><Bot :size="13" /></template>
            </a-button>
          </a-tooltip>
        ...
        <div class="panel-content-viewport">
          ...
          <div v-show="state.activeTab === 'user'" class="tab-pane user-pane">
            <div class="user-cards-grid">
              ...
              <div class="diagnostic-card">
                ...
                <div class="card-body">
                  <div class="user-profile-header">
                    ...
                    <div class="user-title-box">
                      <div class="username-row">
                        ...
                        <span class="role-tag font-mono">{{ assignedRolesText }}</span>
                      </div>
                  ...
                  <div class="info-kv-list">
                    ...
                    <div class="kv-row">
                      <span class="k">角色数量</span>
                      <span class="v font-mono">{{ userStore.userRoles.length }}</span>
                    </div>
                    <div class="kv-row">
                      <span class="k">有效权限数量</span>
                      <span class="v font-mono">{{ userStore.effectivePermissions.length }}</span>
                    </div>
                    <div class="kv-row">
                      <span class="k">Token 状态</span>
                      <span class="v font-mono">
                        {{
                          userStore.token ? '已登录（凭据已隐藏）' : '未登录'
                        }}
                      </span>
                    </div>
                  </div>
              ...
              <!-- 切换用户模拟 (Super Admin) -->
              <div v-if="userStore.hasPermission('user:impersonate')" class="diagnostic-card">
                <div class="card-header">
                    ...
                    >
                      <div class="user-meta-left">
                        ...
                        <div class="user-details">
                          ...
                          <span class="sub font-mono">ID: {{ user.id }} · {{ formatUserRoles(user) }}</span>
                        </div>
                      </div>
...
const tabs = computed(() => [
  ...(userStore.hasPermission('system_log:read')
    ...
    : []),
  ...(userStore.hasPermission('system_config:manage')
...
watch(showModal, (isOpen) => {
  ...
  if (userStore.hasPermission('user:impersonate') && state.users.length === 0) {
    fetchUsers()
  }
})
...
const printDatabaseInfo = () => {
  if (!userStore.hasPermission('knowledge_base:read')) return
  console.log('=== 知识库状态 ===', {
    kbId: databaseStore.kbId,
    databaseName: databaseStore.database?.name,
    fileCount: Object.keys(databaseStore.database?.files || {}).length,
    loading: databaseStore.state?.databaseLoading,
    selectedFileCount: databaseStore.selectedRowKeys?.length || 0
  })
}

const printAgentConfig = () => {
  if (!userStore.hasPermission('agent:manage')) return
  console.log('=== 智能体状态 ===', {
    initialized: agentStore.isInitialized,
    selectedAgentId: agentStore.selectedAgentId,
    agentCount: agentStore.agentsList.length,
    loading: {
      agents: agentStore.isLoadingAgents,
      config: agentStore.isLoadingConfig,
      tools: agentStore.isLoadingTools
    },
    error: agentStore.error,
    configurableItems: Object.keys(agentStore.configurableItems || {}),
    availableTools: Object.keys(agentStore.availableTools || {})
  })
}
...
const sensitiveStorageKeys = new Set(['user_token'])
...
function loadLocalStorageItems() {
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      const byteSize = new Blob([storedValue]).size
      const isSystem = systemKeyPrefixes.some((p) => key.startsWith(p))
      const isSensitive = sensitiveStorageKeys.has(key)
      const value = isSensitive ? '' : storedValue
      let preview = isSensitive ? '***（凭据已隐藏）' : value
...
const reloadAllStores = async () => {
  try {
    const requests = [infoStore.loadInfoConfig(true)]
    if (userStore.hasPermission('system_config:manage')) requests.push(configStore.refreshConfig())
    if (userStore.hasPermission('knowledge_base:read')) requests.push(databaseStore.loadDatabases())
    if (userStore.hasPermission('agent:use')) requests.push(agentStore.initialize())

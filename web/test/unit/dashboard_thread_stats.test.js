...
test('会话审计列表和详情使用最新 Run 状态', () => {
  const listSource = readFileSync(
    new URL('../../src/components/dashboard/ThreadStatsComponent.vue', import.meta.url),
    'utf8'
  )
  const detailSource = readFileSync(
    new URL('../../src/components/dashboard/ThreadDetailDrawer.vue', import.meta.url),
    'utf8'
  )

  assert.match(listSource, /getAgentRunStatusView\(record\.run_status\)/)
  assert.match(detailSource, /getAgentRunStatusView\(detail\?\.run_status\)/)
  assert.match(listSource, /<a-select-option value="active">正常会话<\/a-select-option>/)
})

...
test('Run 刷新会丢弃同路径 artifact 的旧预览并重新读取', () => {
  const source = readFileSync(
    new URL('../../src/components/AgentPanel.vue', import.meta.url),
    'utf8'
  )
  const watcherStart = source.indexOf('() => props.filesystemRefreshVersion')
  const watcherEnd = source.indexOf("watch(\n  () => props.activePreviewPath", watcherStart)
  const refreshWatcher = source.slice(watcherStart, watcherEnd)

  assert.notEqual(watcherStart, -1)
  assert.notEqual(watcherEnd, -1)
  assert.match(refreshWatcher, /!activeTab\.workdir && !activeTab\.workspace/)
  assert.match(refreshWatcher, /invalidatePreviewCacheEntryBeforeReload/)
  assert.match(refreshWatcher, /void loadActivePreview\(\)/)
  assert.ok(
    refreshWatcher.indexOf('invalidatePreviewCacheEntryBeforeReload') <
      refreshWatcher.indexOf('void loadActivePreview()')
  )
})

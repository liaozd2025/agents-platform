...
test('知识库详情将评估基准和近期评估收敛到同一个评估工作台', () => {
  ...
  assert.match(detailSource, /:can-manage="canManageEvaluation"/)
  assert.match(detailSource, /userStore\.hasPermission\('knowledge_evaluation:manage'\)/)
...
test('评估基准和近期评估使用语义按钮导航到可深链接三级页面', () => {
  ...
  assert.equal(
    (routerSource.match(/path: 'knowledgebase\/:kbId\/evaluation\/:datasetId'/g) || []).length,
    2
  )
  assert.match(routerSource, /name: 'ExtensionEvaluationBenchmarkDetail'/)
  assert.match(routerSource, /name: 'EmbedExtensionEvaluationBenchmarkDetail'/)
  assert.match(routerSource, /import\('\.\.\/views\/EvaluationBenchmarkDetailView\.vue'\)/)
  assert.equal(
    (routerSource.match(/requiredPermission: 'knowledge_evaluation:manage'/g) || []).length,
    2
  )
  assert.equal(
    (
      routerSource.match(
        /requiredAnyPermissions: \['knowledge_base:read', 'knowledge_base:manage'\]/g
      ) || []
    ).length,
    4
  )
  assert.doesNotMatch(routerSource, /requiresAdmin/)
  ...
  assert.match(workspaceSource, /route\.matched\.some\(\(record\) => record\.meta\.embed === true\)/)
  assert.match(workspaceSource, /name: benchmarkDetailRouteName\.value/)

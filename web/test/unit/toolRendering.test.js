...
test('子智能体运行失败在专用行和分组摘要中一致展示', async () => {
  for (const toolCall of [
    ...
    const status = toolCall.subagent_run?.status || toolCall.tool_call_result?.content?.active_run_status
    if (status === 'cancelled' || status === 'interrupted') {
      assert.match(row, /tool-loader/)
      assert.match(group, status === 'cancelled' ? /1 已取消/ : /1 已中断/)
    } else {
